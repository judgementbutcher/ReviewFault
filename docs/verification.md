...
| schema v2 | 真实 v1 数据升级、到期保留、重复迁移、类型状态、不可变事件、软删恢复、外键 | 两端各自 SQLite 版本 |
| 备份 v2 | v1/v2 schema、路径与哈希约束；两端源码实现兼容组合和迁移 | v1→v2、v2↔v2 真机互恢复与损坏包 |
...
Windows runner 生成。API 26/35 真机、Windows 10/11 以及两端互恢复仍属于发布后的设备验收。
