...
public sealed class AppRepository
{
            ...
            WHERE s.suspended_at IS NULL AND s.deleted_at IS NULL
              AND ((s.scheduler_state <> 0 AND s.due_at <= $now)
                OR (s.scheduler_state = 0 AND (
                  ...
                  WHERE state_before = 0 AND reviewed_at >= $dayStart
                ) < 20))
            ORDER BY
    ...
    public async Task<ScheduleResult> ReviewAsync(
        ...
        string? mathResult)
    {
        var result = NativeScheduler.Review(
        ...
        await ExecuteAsync(connection, transaction, """
            INSERT INTO review_log (
              id, study_item_id, reviewed_at, rating, duration_seconds,
              scheduler_abi_version, state_before, state_after, elapsed_days,
              scheduled_days, retrievability_before, difficulty_before,
              difficulty_after, stability_before, stability_after, due_at_after,
              device_id, created_at
            ) VALUES (
              $logId, $id, $reviewed, $rating, $duration, 1, $before, $after,
              $elapsed, $scheduled, $retrievability, $difficultyBefore,
              $difficultyAfter, $stabilityBefore, $stabilityAfter, $due, $device, $now)
            """, ("$logId", NewId()), ("$id", row.Id),
            ("$reviewed", reviewedAt), ("$rating", (int)rating),
            ("$duration", Math.Max(0, durationSeconds)), ("$before", (int)row.State),
            ("$after", (int)result.Card.State), ("$elapsed", elapsed),
            ("$scheduled", result.ScheduledDays),
            ...
            ("$due", result.Card.DueAt), ("$device", Environment.MachineName), ("$now", now));
        if (row.Kind == "math_problem" && mathResult is not null)
        {
            await ExecuteAsync(connection, transaction, """
                INSERT INTO attempt (
                  id, math_problem_id, started_at, finished_at, result, created_at
                ) VALUES ($attempt, $id, $started, $finished, $result, $now)
                """, ("$attempt", NewId()), ("$id", row.Id),
                ("$started", reviewedAt - Math.Max(0, durationSeconds)),
                ("$finished", reviewedAt), ("$result", mathResult), ("$now", now));
        }
        await transaction.CommitAsync();
        return result;
    }
    ...
    public async Task RestoreBackupAsync(Stream input)
    {
        ...
        try
        {
            ...
            if (root.GetProperty("format").GetString() != "reviewfault-backup" ||
                ...
                root.GetProperty("schedulerAbiVersion").GetInt32() != 1)
